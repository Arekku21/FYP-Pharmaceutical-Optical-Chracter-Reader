...
reference_string = "panadol"

# Compute the similarity scores using various algorithms
jaro_scores = [textdistance.jaro(reference_string, s) for s in test_strings]
jaro_winkler_scores = [textdistance.jaro_winkler(reference_string, s) for s in test_strings]
levenshtein_scores = [textdistance.levenshtein.normalized_similarity(reference_string, s) for s in test_strings]

# Create a bar chart to compare the similarity scores
fig, ax = plt.subplots()
x = range(len(test_strings))
width = 0.25
...
ax.set_xticklabels(test_strings)
