...
<script>
  $(document).ready(function(){
    ...
    $(".btnRemove").click(function(){
      var choice = confirm("Are you sure want to add to archive?");
      if(choice)
      {
        $.ajax({
          url: "../ajax/ajax.php",
          method: "POST", 
          data: {action: "archiveUser", id: $(".btnRemove").attr("id")} ,
          success: function(result){
            alert(result);
            location.reload();
          }
...
<?php if($_SESSION['roleID'] == 1 || $_SESSION["roleID"] == 2){ ?>
  ...
  <div class="panel-block" style="height: auto">
    ...
    <div style="overflow-y:auto ;overflow-x: hidden;height: 500px;">
                <table id="user_table" class="table is-full" style="width: 100%;">
                  ...
                  <tbody>
                  <?php 
                  $sql = mysqli_query($Links, "SELECT * FROM user WHERE roleID=3 AND status='0'");
                    ...
                    for($i = 0; $i < mysqli_num_rows($sql); $i++)
                    {
                      ...
                      if($_SESSION['roleID'] == 1){
                        ...
                        <td class="level-right">
                          ...
                          <button class="btnRemove button is-small is-danger" style="font-weight: bold;" id="'.$row["id"].'">Archive</button></nobr>
                        </td>
                        </tr>
                        ';
                      }
